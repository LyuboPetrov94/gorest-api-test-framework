...
test.describe("Cross-account data isolation (Users carrier)", () => {
  ...
  test.describe("Nested-list isolation", () => {
    test("TC07 - SUB cannot see MAIN's todos via the nested list (200 [])", async () => {
      // Owner baseline: MAIN sees its todo under uMain - proves the todo exists,
      // so SUB's empty result is isolation, not an empty user.
